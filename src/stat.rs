...
impl Field {
    pub fn new(name: &str, type_: Type) -> Field {
        Field::new_unit(name, type_, Unit::Unknown)
    }
    pub fn new_unit(name: &str, type_: Type, unit: Unit) -> Field {
        ...
        let v = unsafe {
            tll_stat_value_t {
                value: tll_stat_default_int(type_ as tll_stat_type_t),
            }
        };
        Field(tll_stat_field_t {
            type_: t,
            name: n,
            value: v,
        })
    ...
    pub fn type_(&self) -> tll_stat_type_t {
        (self.0.type_ >> 4) & 0xf
    }
    pub fn unit(&self) -> tll_stat_unit_t {
        self.0.type_ & 0xf
    }
...
impl<T: Default> Base<T> {
    ...
    pub fn as_ptr(&mut self) -> *mut tll_stat_block_t {
        &mut self.block
    }
    ...
    pub fn acquire(&mut self) -> Option<Reference<'_, T>> {
        ...
        Some(Reference {
            data: &mut self.data[if p == &mut self.data[0].page { 0 } else { 1 }],
            base: &mut self.block,
        })
    }

    pub fn release(&mut self, page: &T) {
        let p = &mut self.data[if page as *const T == &self.data[0].data as *const T {
            0
        } else {
            1
        }];
