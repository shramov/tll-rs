...
pub fn main() -> tll::result::Result<()> {
    ...
    let _netlink_cb = netlink.callback_add_mut::<SystemState, RouteCallback>(&mut state, MsgMask::Data)?;

    let mut nl80211 = ctx.channel("nl80211://;name=nl80211;dump=scheme;addr=no;neigh=no")?;
    let _nl80211_cb = nl80211.callback_add_mut::<SystemState, NL80211Callback>(&mut state, MsgMask::Data)?;

    let mut udev = ctx.channel("udev://;name=udev;dump=scheme;subsystem=power_supply")?;
    let _udev_cb = udev.callback_add_mut::<SystemState, PowerCallback>(&mut state, MsgMask::Data)?;
    ...
    let _timer_cb = tc.callback_add_mut::<SystemState, TimerCallback>(&mut state, MsgMask::Data)?;
